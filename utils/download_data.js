...
const repo = require('../repositories/sireneRepository');
...
function makeRequest(param, refreshObj) {
  ...
  try {
    ...
    result.then(function (table) { //SUCCESS CALLBACK
      ...
      tableResultObject.offset = JSON.parse(lastSiret);
      tableResultObject.sirene = JSON.parse(JSONstr);
...
function refreshObj(paramObj, resultSent) {
  ...
  nblignestot += resultSent.sirene.length;
